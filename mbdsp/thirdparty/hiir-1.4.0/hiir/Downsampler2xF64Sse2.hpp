...
{
    for(int i = 0; i < _nbr_stages + 1; ++i) { _mm_store_pd(_filter[i]._coef, _mm_setzero_pd()); }
    if(NBR_COEFS < _nbr_stages * 2) { _filter[_nbr_stages]._coef[0] = 1; }
...
template <int NC>
void Downsampler2xF64Sse2<NC>::process_block(double out_ptr[], const double in_ptr[],
                                             long nbr_spl) noexcept
{
    ...
    const long n2 = process_block_double(out_ptr, nullptr, in_ptr, nbr_spl, store_low, bypass);
    if(n2 < nbr_spl) { out_ptr[n2] = process_sample(in_ptr + n2 * 2); }
}
...
template <int NC>
void Downsampler2xF64Sse2<NC>::process_sample_split(double &low, double &high,
                                                    const double in_ptr[2]) noexcept
{
    assert(in_ptr != nullptr);
...
template <int NC>
void Downsampler2xF64Sse2<NC>::process_block_split(double out_l_ptr[], double out_h_ptr[],
                                                   const double in_ptr[], long nbr_spl) noexcept
{
    assert(in_ptr != nullptr);
    ...
    const long n2 =
        process_block_double(out_l_ptr, out_h_ptr, in_ptr, nbr_spl, store_low, store_high);
    if(n2 < nbr_spl) { process_sample_split(out_l_ptr[n2], out_h_ptr[n2], in_ptr + n2 * 2); }
}
...
{
    for(int i = 0; i < _nbr_stages + 1; ++i) { _mm_store_pd(_filter[i]._mem, _mm_setzero_pd()); }
}
...
long Downsampler2xF64Sse2<NC>::process_block_double(double out_l_ptr[], double out_h_ptr[],
                                                    const double in_ptr[], long nbr_spl, FL fnc_l,
                                                    FH fnc_h) noexcept
{
...
template <int NC>
void Downsampler2xF64Sse2<NC>::store_low(double *ptr, __m128d even, __m128d odd,
                                         __m128d half) noexcept
{
...
template <int NC>
void Downsampler2xF64Sse2<NC>::store_high(double *ptr, __m128d even, __m128d odd,
                                          __m128d half) noexcept
{
