...
template <int NC>
HalfBandNeon<NC>::HalfBandNeon() noexcept : _filter(), _prev(0), _phase(0)
{
...
void HalfBandNeon<NC>::process_block(float out_ptr[], const float in_ptr[], long nbr_spl) noexcept
{
    ...
    if(nbr_spl > 0)
    {
        ...
        for(long pos = n4; pos < nbr_spl; ++pos) { out_ptr[pos] = process_sample(in_ptr[pos]); }
    }
}
...
template <int NC>
void HalfBandNeon<NC>::process_block_hpf(float out_ptr[], const float in_ptr[],
                                         long nbr_spl) noexcept
{
    ...
    if(nbr_spl > 0)
    {
        const long n4 = process_block_quad(nullptr, out_ptr, in_ptr, nbr_spl, bypass, store_high);

        for(long pos = n4; pos < nbr_spl; ++pos) { out_ptr[pos] = process_sample_hpf(in_ptr[pos]); }
    }
}
...
template <int NC>
void HalfBandNeon<NC>::process_block_split(float out_l_ptr[], float out_h_ptr[],
...
void HalfBandNeon<NC>::clear_buffers() noexcept
{
    for(int phase = 0; phase < _nbr_phases; ++phase)
    {
        for(int i = 0; i < _nbr_stages + 1; ++i) { storea(_filter[phase][i]._mem, vdupq_n_f32(0)); }
    }
}
...
long HalfBandNeon<NC>::process_block_quad(float out_l_ptr[], float out_h_ptr[],
...
template <int NC>
void HalfBandNeon<NC>::store_low(float *ptr, float32x4_t even, float32x4_t odd,
...
template <int NC>
void HalfBandNeon<NC>::store_high(float *ptr, float32x4_t even, float32x4_t odd,
                                  float32x4_t half) noexcept
{
