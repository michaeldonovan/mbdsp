...
template <int NC>
Downsampler2x4Sse<NC>::Downsampler2x4Sse() noexcept : _filter()
{
    for(int i = 0; i < NBR_COEFS + 2; ++i) { _mm_store_ps(_filter[i]._coef, _mm_setzero_ps()); }
...
__m128 Downsampler2x4Sse<NC>::process_sample(__m128 in_0, __m128 in_1) noexcept
{
    ...
    StageProc4Sse<NBR_COEFS>::process_sample_pos(NBR_COEFS, spl_0, spl_1, &_filter[0]);
...
template <int NC>
void Downsampler2x4Sse<NC>::process_block(float out_ptr[], const float in_ptr[],
...
template <int NC>
void Downsampler2x4Sse<NC>::process_sample_split(__m128 &low, __m128 &high,
...
template <int NC>
void Downsampler2x4Sse<NC>::process_sample_split(__m128 &low, __m128 &high, __m128 in_0,
...
template <int NC>
void Downsampler2x4Sse<NC>::process_block_split(float out_l_ptr[], float out_h_ptr[],
...
{
    for(int i = 0; i < NBR_COEFS + 2; ++i) { _mm_store_ps(_filter[i]._mem, _mm_setzero_ps()); }
}
