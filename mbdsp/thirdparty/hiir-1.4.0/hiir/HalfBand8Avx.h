...
public:
    ...
    HalfBand8Avx<NC> &operator=(const HalfBand8Avx<NC> &other) = default;
    HalfBand8Avx<NC> &operator=(HalfBand8Avx<NC> &&other) = default;

    void set_coefs(const double coef_arr[]) noexcept;

    hiir_FORCEINLINE __m256 process_sample(__m256 input) noexcept;
    void process_block(float out_ptr[], const float in_ptr[], long nbr_spl) noexcept;

    hiir_FORCEINLINE __m256 process_sample_hpf(__m256 input) noexcept;
    void process_block_hpf(float out_ptr[], const float in_ptr[], long nbr_spl) noexcept;

    hiir_FORCEINLINE void process_sample_split(__m256 &low, __m256 &high, __m256 input) noexcept;
...
private:
    ...
    typedef std::array<StageDataAvx, NBR_COEFS + 2>
    ...
    hiir_FORCEINLINE EvenOdd process_2_paths(__m256 input) noexcept;
    ...
    hiir_FORCEINLINE static void store_low(float *ptr, __m256 tmp_0, __m256 tmp_1) noexcept;
    hiir_FORCEINLINE static void store_high(float *ptr, __m256 tmp_0, __m256 tmp_1) noexcept;
    hiir_FORCEINLINE static void bypass(float *, __m256, __m256) noexcept {}

    FilterBiPhase _bifilter;
    alignas(32) float _prev[_nbr_chn];
    int _phase;  // 0 or 1
