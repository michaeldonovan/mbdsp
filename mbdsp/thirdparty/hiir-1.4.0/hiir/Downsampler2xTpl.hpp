...
void Downsampler2xTpl<NC, DT, NCHN>::set_coefs(const double coef_arr[]) noexcept
{
    ...
    for(int i = 0; i < NBR_COEFS; ++i) { _filter[i + 2]._coef = DataType(coef_arr[i]); }
}
...
template <int NC, typename DT, int NCHN>
typename Downsampler2xTpl<NC, DT, NCHN>::DataType Downsampler2xTpl<NC, DT, NCHN>::process_sample(
    const DataType in_ptr[2]) noexcept
{
    ...
    StageProcTpl<NBR_COEFS, DataType>::process_sample_pos(NBR_COEFS, spl_0, spl_1, _filter.data());
...
template <int NC, typename DT, int NCHN>
void Downsampler2xTpl<NC, DT, NCHN>::process_block(DataType out_ptr[], const DataType in_ptr[],
...
template <int NC, typename DT, int NCHN>
void Downsampler2xTpl<NC, DT, NCHN>::process_sample_split(DataType &low, DataType &high,
                                                          const DataType in_ptr[2]) noexcept
{
    assert(in_ptr != nullptr);
...
template <int NC, typename DT, int NCHN>
void Downsampler2xTpl<NC, DT, NCHN>::process_block_split(DataType out_l_ptr[], DataType out_h_ptr[],
                                                         const DataType in_ptr[],
                                                         long nbr_spl) noexcept
{
    assert(in_ptr != nullptr);
...
{
    for(int i = 0; i < NBR_COEFS + 2; ++i) { _filter[i]._mem = DataType(0.f); }
}
