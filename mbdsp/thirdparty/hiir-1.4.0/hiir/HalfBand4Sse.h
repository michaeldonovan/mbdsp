...
public:
    ...
    HalfBand4Sse<NC> &operator=(const HalfBand4Sse<NC> &other) = default;
    HalfBand4Sse<NC> &operator=(HalfBand4Sse<NC> &&other) = default;

    void set_coefs(const double coef_arr[]) noexcept;

    hiir_FORCEINLINE __m128 process_sample(__m128 input) noexcept;
    void process_block(float out_ptr[], const float in_ptr[], long nbr_spl) noexcept;

    hiir_FORCEINLINE __m128 process_sample_hpf(__m128 input) noexcept;
    void process_block_hpf(float out_ptr[], const float in_ptr[], long nbr_spl) noexcept;

    hiir_FORCEINLINE void process_sample_split(__m128 &low, __m128 &high, __m128 input) noexcept;
...
private:
    ...
    hiir_FORCEINLINE EvenOdd process_2_paths(__m128 input) noexcept;
    ...
    hiir_FORCEINLINE static void store_low(float *ptr, __m128 tmp_0, __m128 tmp_1) noexcept;
    hiir_FORCEINLINE static void store_high(float *ptr, __m128 tmp_0, __m128 tmp_1) noexcept;
    hiir_FORCEINLINE static void bypass(float *, __m128, __m128) noexcept {}
