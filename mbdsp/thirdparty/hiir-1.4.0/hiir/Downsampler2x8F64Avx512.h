...
public:
    ...
    Downsampler2x8F64Avx512<NC> &operator=(const Downsampler2x8F64Avx512<NC> &other) = default;
    Downsampler2x8F64Avx512<NC> &operator=(Downsampler2x8F64Avx512<NC> &&other) = default;

    void set_coefs(const double coef_arr[]) noexcept;

    hiir_FORCEINLINE __m512d process_sample(const double in_ptr[_nbr_chn * 2]) noexcept;
    hiir_FORCEINLINE __m512d process_sample(__m512d in_0, __m512d in_1) noexcept;
    void process_block(double out_ptr[], const double in_ptr[], long nbr_spl) noexcept;

    hiir_FORCEINLINE void process_sample_split(__m512d &low, __m512d &high,
                                               const double in_ptr[_nbr_chn * 2]) noexcept;
    hiir_FORCEINLINE void process_sample_split(__m512d &low, __m512d &high, __m512d in_0,
