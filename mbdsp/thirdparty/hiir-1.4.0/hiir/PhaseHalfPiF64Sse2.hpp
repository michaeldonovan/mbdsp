...
template <int NC>
PhaseHalfPiF64Sse2<NC>::PhaseHalfPiF64Sse2() noexcept : _bifilter(), _prev(0), _phase(0)
{
...
void PhaseHalfPiF64Sse2<NC>::process_sample(double &out_0, double &out_1, double input) noexcept
{
    ...
    StageProcF64Sse2<_nbr_stages>::process_sample_neg(x, &_bifilter[_phase][0]);
...
template <int NC>
void PhaseHalfPiF64Sse2<NC>::process_block(double out_0_ptr[], double out_1_ptr[],
                                           const double in_ptr[], long nbr_spl) noexcept
{
    ...
    while(pos < end)
    {
        ...
        StageProcF64Sse2<_nbr_stages>::process_sample_neg(x_0, &_bifilter[0][0]);

        const auto input_1 = _mm_set1_pd(in_ptr[pos + 1]);
        auto x_1 = _mm_shuffle_pd(input_0, input_1, 1);  // prev = input_0
        StageProcF64Sse2<_nbr_stages>::process_sample_neg(x_1, &_bifilter[1][0]);
