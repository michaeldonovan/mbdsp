...
template <class TO>
int TestDownsampler2x<TO>::perform_test(TO &dspl, const double coef_arr[_nbr_coefs],
                                        const SweepingSine &ss, const char *type_0,
                                        double transition_bw, double stopband_at)
{
    ...
    const char *datatype_0 = std::is_same<DataType, double>::value  ? "double"
                             : std::is_same<DataType, float>::value ? "float"
                                                                    : "<unknown type>";
    printf("Test: Downsampler2x, %s, %d chn, %s implementation, %d coefficients.\n", datatype_0,
    ...
    for(bs.start(len_proc); bs.is_continuing(); bs.set_next_block())
    {
        ...
        dspl.process_block(&dest_lo[b_pos * _nbr_chn], &src[b_pos * 2 * _nbr_chn], b_len);
    }
    printf("Done.\n");
    ...
    if(ret_val == 0)
    {
        ...
        for(bs.start(len_proc); bs.is_continuing(); bs.set_next_block())
        {
            const long b_pos = bs.get_pos();
            const long b_len = bs.get_len();
            dspl.process_block_split(&dest_lo[b_pos * _nbr_chn], &dest_hi[b_pos * _nbr_chn],
                                     &src[b_pos * 2 * _nbr_chn], b_len);
        }
        printf("Done.\n");
    }
...
template <class TO>
int TestDownsampler2x<TO>::check_band(std::vector<DataType> &dst_chk,
                                      const std::vector<DataType> &dest, bool hi_flag, int chn,
                                      const SweepingSine &ss, const char *type_0,
                                      double transition_bw, double stopband_at,
