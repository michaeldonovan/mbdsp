...
template <class AP>
SpeedTester<AP>::SpeedTester() noexcept : _tested_object()
{
    // Nothing
}
...
void SpeedTester<AP>::perform_test(const char *classname_0, const char *funcname_0) noexcept
{
    ...
    printf("Speed test, %-23s::%-19s, %2d coef:", classname_0, funcname_0, NBR_COEFS);
    fflush(stdout);

    // Prepares the filter
    const double transition_bw = 0.1 / NBR_COEFS;
    double coef_arr[NBR_COEFS];
    hiir::PolyphaseIir2Designer::compute_coefs_spec_order_tbw(&coef_arr[0], NBR_COEFS,
                                                              transition_bw);
    ...
    // Displays result
    const double spl_per_s = tim.get_best_rate(this->_block_len * this->_nbr_blocks);
