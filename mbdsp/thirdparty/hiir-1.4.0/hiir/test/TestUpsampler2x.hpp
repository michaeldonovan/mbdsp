...
template <class TO>
int TestUpsampler2x<TO>::perform_test(TO &uspl, const double coef_arr[_nbr_coefs],
                                      ...
                                      double transition_bw, double stopband_at)
{
    ...
    const char *datatype_0 = std::is_same<DataType, double>::value  ? "double"
                             : std::is_same<DataType, float>::value ? "float"
                                                                    : "<unknown type>";
    printf("Test: Upsampler2x, %s, %d chn, %s implementation, %d coefficients.\n", datatype_0,
    ...
    for(bs.start(len); bs.is_continuing(); bs.set_next_block())
    {
        ...
        uspl.process_block(&dest[b_pos * 2 * _nbr_chn], &src[b_pos * _nbr_chn], b_len);
    }
    printf("Done.\n");
    ...
    {
        for(long pos = 0; pos < len_save; ++pos) { dst_chk[pos] = dest[pos * _nbr_chn + chn]; }
        ret_val = ResultCheck<DataType>::check_uspl(ss, transition_bw, stopband_at, &dst_chk[0]);

        char filename_0[255 + 1];
        sprintf(filename_0, "uspl_%02d_%s_%dx-%01d.raw", _nbr_coefs, type_0, _nbr_chn, chn);
        FileOp<DataType>::save_raw_data(filename_0, dst_chk.data(), len_save, hiir_test_file_resol,
                                        1.f);
    }
