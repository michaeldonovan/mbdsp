...
void BlockSplitter::start(long total_len)
{
    ...
    _pos = 0;
    for(int i = 0; i < NBR_LEN; ++i) { _cur_len[i] = 1; }
    _len_index = 0;
}

bool BlockSplitter::is_continuing() const { return (_pos < _total_len); }
...
void BlockSplitter::set_next_block()
{
    ...
    if(_len_index >= NBR_LEN)
    {
        ...
        {
            ++_cur_len[i];
            if(_cur_len[i] > _max_block_len) { _cur_len[i] = 1; }
            else { cont_flag = false; }
        }
    }
}

long BlockSplitter::get_pos() const { return _pos; }

long BlockSplitter::get_len() const
{
    long len = _cur_len[_len_index];
    if(_pos + len > _total_len) { len = _total_len - _pos; }
    assert(len > 0);
