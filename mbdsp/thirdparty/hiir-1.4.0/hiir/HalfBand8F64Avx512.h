...
public:
    ...
    HalfBand8F64Avx512<NC> &operator=(const HalfBand8F64Avx512<NC> &other) = default;
    HalfBand8F64Avx512<NC> &operator=(HalfBand8F64Avx512<NC> &&other) = default;

    void set_coefs(const double coef_arr[]) noexcept;

    hiir_FORCEINLINE __m512d process_sample(__m512d input) noexcept;
    void process_block(double out_ptr[], const double in_ptr[], long nbr_spl) noexcept;

    hiir_FORCEINLINE __m512d process_sample_hpf(__m512d input) noexcept;
    void process_block_hpf(double out_ptr[], const double in_ptr[], long nbr_spl) noexcept;

    hiir_FORCEINLINE void process_sample_split(__m512d &low, __m512d &high, __m512d input) noexcept;
    void process_block_split(double out_l_ptr[], double out_h_ptr[], const double in_ptr[],
...
private:
    ...
    typedef std::array<StageDataF64Avx512, NBR_COEFS + 2>
    ...
    hiir_FORCEINLINE EvenOdd process_2_paths(__m512d input) noexcept;
    template <typename FL, typename FH>
    hiir_FORCEINLINE void process_block_2_paths(double out_l_ptr[], double out_h_ptr[],
                                                const double in_ptr[], long nbr_spl, FL fnc_l,
                                                FH fnc_h) noexcept;

    hiir_FORCEINLINE static void store_low(double *ptr, __m512d tmp_0, __m512d tmp_1) noexcept;
    hiir_FORCEINLINE static void store_high(double *ptr, __m512d tmp_0, __m512d tmp_1) noexcept;
    hiir_FORCEINLINE static void bypass(double *, __m512d, __m512d) noexcept {}

    FilterBiPhase _bifilter;
    alignas(64) double _prev[_nbr_chn];
    int _phase;  // 0 or 1
