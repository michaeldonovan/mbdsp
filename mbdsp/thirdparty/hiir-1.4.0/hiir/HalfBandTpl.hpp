...
template <int NC, typename DT, int NCHN>
typename HalfBandTpl<NC, DT, NCHN>::DataType HalfBandTpl<NC, DT, NCHN>::process_sample(
    DataType in) noexcept
{
...
template <int NC, typename DT, int NCHN>
void HalfBandTpl<NC, DT, NCHN>::process_block(DataType out_ptr[], const DataType in_ptr[],
                                              long nbr_spl) noexcept
{
    ...
    for(long pos = 0; pos < nbr_spl; ++pos) { out_ptr[pos] = process_sample(in_ptr[pos]); }
}
...
template <int NC, typename DT, int NCHN>
typename HalfBandTpl<NC, DT, NCHN>::DataType HalfBandTpl<NC, DT, NCHN>::process_sample_hpf(
    DataType in) noexcept
{
    const auto path_arr = process_2_paths(in);
    const auto out = (path_arr[0] - path_arr[1]) * DataType(0.5f);
...
template <int NC, typename DT, int NCHN>
void HalfBandTpl<NC, DT, NCHN>::process_block_hpf(DataType out_ptr[], const DataType in_ptr[],
                                                  long nbr_spl) noexcept
{
    assert(in_ptr != nullptr);
    assert(out_ptr != nullptr);
    ...
    for(long pos = 0; pos < nbr_spl; ++pos) { out_ptr[pos] = process_sample_hpf(in_ptr[pos]); }
}
...
template <int NC, typename DT, int NCHN>
void HalfBandTpl<NC, DT, NCHN>::process_sample_split(DataType &low, DataType &high,
                                                     DataType in) noexcept
{
    const auto path_arr = process_2_paths(in);
...
template <int NC, typename DT, int NCHN>
void HalfBandTpl<NC, DT, NCHN>::process_block_split(DataType out_l_ptr[], DataType out_h_ptr[],
                                                    const DataType in_ptr[], long nbr_spl) noexcept
{
    assert(in_ptr != nullptr);
...
template <int NC, typename DT, int NCHN>
std::array<typename HalfBandTpl<NC, DT, NCHN>::DataType, 2>
HalfBandTpl<NC, DT, NCHN>::process_2_paths(DataType input) noexcept
{
    std::array<DataType, 2> path_arr{input, _prev};

    StageProcTpl<NBR_COEFS, DataType>::process_sample_pos(NBR_COEFS, path_arr[0], path_arr[1],
                                                          _bifilter[_phase].data());
