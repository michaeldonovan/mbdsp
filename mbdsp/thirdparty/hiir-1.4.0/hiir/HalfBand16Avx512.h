...
public:
    ...
    HalfBand16Avx512<NC> &operator=(const HalfBand16Avx512<NC> &other) = default;
    HalfBand16Avx512<NC> &operator=(HalfBand16Avx512<NC> &&other) = default;

    void set_coefs(const double coef_arr[]) noexcept;

    hiir_FORCEINLINE __m512 process_sample(__m512 input) noexcept;
    void process_block(float out_ptr[], const float in_ptr[], long nbr_spl) noexcept;

    hiir_FORCEINLINE __m512 process_sample_hpf(__m512 input) noexcept;
    void process_block_hpf(float out_ptr[], const float in_ptr[], long nbr_spl) noexcept;

    hiir_FORCEINLINE void process_sample_split(__m512 &low, __m512 &high, __m512 input) noexcept;
...
private:
    ...
    typedef std::array<StageDataAvx512, NBR_COEFS + 2>
    ...
    hiir_FORCEINLINE EvenOdd process_2_paths(__m512 input) noexcept;
    ...
    hiir_FORCEINLINE static void store_low(float *ptr, __m512 tmp_0, __m512 tmp_1) noexcept;
    hiir_FORCEINLINE static void store_high(float *ptr, __m512 tmp_0, __m512 tmp_1) noexcept;
    hiir_FORCEINLINE static void bypass(float *, __m512, __m512) noexcept {}

    FilterBiPhase _bifilter;
    alignas(64) float _prev[_nbr_chn];
    int _phase;  // 0 or 1
