...
public:
    ...
    hiir_FORCEINLINE DataType process_sample(DataType in) noexcept;
    void process_block(DataType out_ptr[], const DataType in_ptr[], long nbr_spl) noexcept;

    hiir_FORCEINLINE DataType process_sample_hpf(DataType in) noexcept;
    void process_block_hpf(DataType out_ptr[], const DataType in_ptr[], long nbr_spl) noexcept;

    hiir_FORCEINLINE void process_sample_split(DataType &low, DataType &high, DataType in) noexcept;
    void process_block_split(DataType out_l_ptr[], DataType out_h_ptr[], const DataType in_ptr[],
...
private:
    ...
    hiir_FORCEINLINE std::array<DataType, 2> process_2_paths(DataType input) noexcept;
