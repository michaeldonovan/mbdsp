...
public:
    ...
    Downsampler2x4F64Avx<NC> &operator=(const Downsampler2x4F64Avx<NC> &other) = default;
    Downsampler2x4F64Avx<NC> &operator=(Downsampler2x4F64Avx<NC> &&other) = default;

    void set_coefs(const double coef_arr[]) noexcept;

    hiir_FORCEINLINE __m256d process_sample(const double in_ptr[_nbr_chn * 2]) noexcept;
    hiir_FORCEINLINE __m256d process_sample(__m256d in_0, __m256d in_1) noexcept;
    void process_block(double out_ptr[], const double in_ptr[], long nbr_spl) noexcept;

    hiir_FORCEINLINE void process_sample_split(__m256d &low, __m256d &high,
                                               const double in_ptr[_nbr_chn * 2]) noexcept;
    hiir_FORCEINLINE void process_sample_split(__m256d &low, __m256d &high, __m256d in_0,
                                               __m256d in_1) noexcept;
