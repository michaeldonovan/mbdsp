    ...
    F compute_delay) noexcept
{
    ...
    // could be met
    if((ub_gdly - delay) * (delay - lb_gdly) <= 0) { ret_val = ResCode_FAIL_GD_TBW; }
    ...
    if(ret_val == ResCode_OK)
    {
        ...
        // Checks convergence
        if(nbr_it >= max_it && !conv_flag) { ret_val = ResCode_FAIL_CONV; }

        // Checks the attenuation requirement
        else if(rs_attn < atten_lb || rs_attn > atten_ub) { ret_val = ResCode_FAIL_ATTEN; }
    }

    if(ret_val == ResCode_OK)
    {
        if(attenuation_ptr != nullptr) { *attenuation_ptr = rs_attn; }
        if(transition_ptr != nullptr) { *transition_ptr = rs_tbw; }
    }
