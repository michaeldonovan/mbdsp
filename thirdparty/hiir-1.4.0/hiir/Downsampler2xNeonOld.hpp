...
template <int NC>
Downsampler2xNeonOld<NC>::Downsampler2xNeonOld() noexcept : _filter()
{
    for(int i = 0; i < _nbr_stages + 1; ++i) { storea(_filter[i]._mem, vdupq_n_f32(0)); }
    for(int i = NBR_COEFS; i < _nbr_stages * _stage_width; ++i) { set_single_coef(i, 1); }
...
void Downsampler2xNeonOld<NC>::set_coefs(const double coef_arr[]) noexcept
{
    ...
    for(int i = 0; i < NBR_COEFS; ++i) { set_single_coef(i, coef_arr[i]); }
}
...
float Downsampler2xNeonOld<NC>::process_sample(const float in_ptr[2]) noexcept
{
    ...
    const auto spl_in = vreinterpret_f32_u8(vld1_u8(reinterpret_cast<const uint8_t *>(in_ptr)));
...
template <int NC>
void Downsampler2xNeonOld<NC>::process_block(float out_ptr[], const float in_ptr[],
                                             long nbr_spl) noexcept
{
    ...
    const long n4 = process_block_quad(out_ptr, nullptr, in_ptr, nbr_spl, store_low, bypass);

    for(long pos = n4; pos < nbr_spl; ++pos) { out_ptr[pos] = process_sample(in_ptr + pos * 2); }
}
...
template <int NC>
void Downsampler2xNeonOld<NC>::process_sample_split(float &low, float &high,
                                                    const float in_ptr[2]) noexcept
{
    assert(in_ptr != nullptr);
...
template <int NC>
void Downsampler2xNeonOld<NC>::process_block_split(float out_l_ptr[], float out_h_ptr[],
                                                   const float in_ptr[], long nbr_spl) noexcept
{
    assert(in_ptr != nullptr);
    ...
    const long n4 =
        process_block_quad(out_l_ptr, out_h_ptr, in_ptr, nbr_spl, store_low, store_high);
...
long Downsampler2xNeonOld<NC>::process_block_quad(float out_l_ptr[], float out_h_ptr[],
                                                  const float in_ptr[], long nbr_spl, FL fnc_l,
                                                  FH fnc_h) noexcept
{
    assert(_phase == 0);
...
template <int NC>
void Downsampler2xNeonOld<NC>::store_low(float *ptr, float32x4_t even, float32x4_t odd,
                                         float32x4_t half) noexcept
{
...
template <int NC>
void Downsampler2xNeonOld<NC>::store_high(float *ptr, float32x4_t even, float32x4_t odd,
                                          float32x4_t half) noexcept
{
