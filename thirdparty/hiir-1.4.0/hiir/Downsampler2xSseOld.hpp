...
template <int NC>
Downsampler2xSseOld<NC>::Downsampler2xSseOld() noexcept : _filter()
{
    for(int i = 0; i < _nbr_stages + 1; ++i) { _mm_store_ps(_filter[i]._coef, _mm_setzero_ps()); }
...
template <int NC>
void Downsampler2xSseOld<NC>::process_block(float out_ptr[], const float in_ptr[],
...
template <int NC>
void Downsampler2xSseOld<NC>::process_sample_split(float &low, float &high,
...
template <int NC>
void Downsampler2xSseOld<NC>::process_block_split(float out_l_ptr[], float out_h_ptr[],
...
{
    for(int i = 0; i < _nbr_stages + 1; ++i) { _mm_store_ps(_filter[i]._mem, _mm_setzero_ps()); }
}
...
long Downsampler2xSseOld<NC>::process_block_quad(float out_l_ptr[], float out_h_ptr[],
