...
template <int NC>
Downsampler2x3dnow<NC>::Downsampler2x3dnow() noexcept : _filter()
{
    ...
    if(NBR_COEFS < _nbr_stages * 2) { _filter[_nbr_stages]._coefs.m64_f32[0] = 1; }
...
template <int NC>
void Downsampler2x3dnow<NC>::process_block(float out_ptr[], const float in_ptr[],
                                           long nbr_spl) noexcept
{
    ...
    __asm pop eax
        ...
        __asm
    {
        movq[edx + CURR_CELL + 1 * 8],
            mm0

                inc eax pfmul mm0,
            Downsampler2x3dnow_half pfacc mm0, mm0 movd[edi + eax * 4],
            mm0
...
template <>
void Downsampler2x3dnow<8>::process_block(float out_ptr[], const float in_ptr[],
                                          long nbr_spl) noexcept
{
    StageData3dnow *filter_ptr = &_filter[0];
...
template <int NC>
void Downsampler2x3dnow<NC>::process_sample_split(float &low, float &high,
...
template <int NC>
void Downsampler2x3dnow<NC>::process_block_split(float out_l_ptr[], float out_h_ptr[],
...
#if defined(_MSC_VER) && !defined(NDEBUG)
    __asm push eax __asm push ecx
#endif
    StageProc3dnow<_nbr_stages>::process_sample_pos();
#if defined(_MSC_VER) && !defined(NDEBUG)
    __asm pop ecx __asm pop eax
#endif
        __asm
    {
        movq[edx + CURR_CELL + 1 * 8],
            mm0
                ...
                movq mm1,
            mm0 inc eax pfmul mm0, Downsampler2x3dnow_half pfacc mm0, mm0 movd[edi + eax * 4],
            mm0 pfsub mm1, mm0 movd[ecx + eax * 4],
            mm1
