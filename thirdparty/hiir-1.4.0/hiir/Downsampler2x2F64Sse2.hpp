...
template <int NC>
Downsampler2x2F64Sse2<NC>::Downsampler2x2F64Sse2() noexcept : _filter()
{
    for(int i = 0; i < NBR_COEFS + 2; ++i) { _mm_store_pd(_filter[i]._coef, _mm_setzero_pd()); }
...
void Downsampler2x2F64Sse2<NC>::set_coefs(const double coef_arr[]) noexcept
{
    ...
    {
        _mm_store_pd(_filter[i + 2]._coef, _mm_set1_pd(DataType(coef_arr[i])));
    }
}
...
__m128d Downsampler2x2F64Sse2<NC>::process_sample(__m128d in_0, __m128d in_1) noexcept
{
    ...
    StageProc2F64Sse2<NBR_COEFS>::process_sample_pos(NBR_COEFS, spl_0, spl_1, &_filter[0]);
...
template <int NC>
void Downsampler2x2F64Sse2<NC>::process_block(double out_ptr[], const double in_ptr[],
...
template <int NC>
void Downsampler2x2F64Sse2<NC>::process_sample_split(__m128d &low, __m128d &high,
                                                     const double in_ptr[_nbr_chn * 2]) noexcept
{
    assert(in_ptr != nullptr);
...
template <int NC>
void Downsampler2x2F64Sse2<NC>::process_sample_split(__m128d &low, __m128d &high, __m128d in_0,
                                                     __m128d in_1) noexcept
{
    __m128d spl_0 = in_1;
    __m128d spl_1 = in_0;
...
template <int NC>
void Downsampler2x2F64Sse2<NC>::process_block_split(double out_l_ptr[], double out_h_ptr[],
...
{
    for(int i = 0; i < NBR_COEFS + 2; ++i) { _mm_store_pd(_filter[i]._mem, _mm_setzero_pd()); }
}
