...
template <int NC>
Upsampler2xNeon<NC>::Upsampler2xNeon() noexcept : _filter()
{
    for(int i = 0; i < _nbr_stages + 1; ++i) { storea(_filter[i]._coef, vdup_n_f32(0)); }
    if((NBR_COEFS & 1) != 0)
    {
...
template <int NC>
void Upsampler2xNeon<NC>::process_block(float out_ptr[], const float in_ptr[],
...
template <int NC>
long Upsampler2xNeon<NC>::process_block_quad(float out_ptr[], const float in_ptr[],
                                             long nbr_spl) noexcept
{
