...
template <int NC>
PhaseHalfPi3dnow<NC>::PhaseHalfPi3dnow() noexcept : _filter(), _prev(0), _phase(0)
{
    for(int phase = 0; phase < _nbr_phases; ++phase)
    {
        ...
        if(NBR_COEFS < _nbr_stages * 2) { _filter[phase][_nbr_stages]._coefs.m64_f32[0] = 1; }
    }
...
template <int NC>
void PhaseHalfPi3dnow<NC>::process_block(float out_0_ptr[], float out_1_ptr[], const float in_ptr[],
...
#endif
            __asm {inc eax movq[edx + CURR_CELL + 1 * 8],
                   mm0 xchg edx,
                   ebx movd[ecx + eax * 4],
                   mm0 punpckhdq mm0,
                   mm0 movd[edi + eax * 4],
