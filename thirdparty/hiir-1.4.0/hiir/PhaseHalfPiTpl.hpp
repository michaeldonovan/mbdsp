...
template <int NC, typename DT, int NCHN>
void PhaseHalfPiTpl<NC, DT, NCHN>::process_sample(DataType &out_0, DataType &out_1,
                                                  DataType input) noexcept
{
    out_0 = input;  // Even coefs
    out_1 = _prev;  // Odd coefs

    StageProcTpl<NBR_COEFS, DataType>::process_sample_neg(NBR_COEFS, out_0, out_1,
...
template <int NC, typename DT, int NCHN>
void PhaseHalfPiTpl<NC, DT, NCHN>::process_block(DataType out_0_ptr[], DataType out_1_ptr[],
                                                 const DataType in_ptr[], long nbr_spl) noexcept
{
    ...
    while(pos < end)
    {
        ...
        out_1_ptr[pos] = _prev;
        StageProcTpl<NBR_COEFS, DataType>::process_sample_neg(NBR_COEFS, out_0_ptr[pos],
                                                              out_1_ptr[pos], _bifilter[0].data());
        ...
        StageProcTpl<NBR_COEFS, DataType>::process_sample_neg(
            NBR_COEFS, out_0_ptr[pos + 1], out_1_ptr[pos + 1], _bifilter[1].data());
        _prev = input_1;
