...
public:
    ...
    HalfBand4F64Avx<NC> &operator=(const HalfBand4F64Avx<NC> &other) = default;
    HalfBand4F64Avx<NC> &operator=(HalfBand4F64Avx<NC> &&other) = default;

    void set_coefs(const double coef_arr[]) noexcept;

    hiir_FORCEINLINE __m256d process_sample(__m256d input) noexcept;
    void process_block(double out_ptr[], const double in_ptr[], long nbr_spl) noexcept;

    hiir_FORCEINLINE __m256d process_sample_hpf(__m256d input) noexcept;
    void process_block_hpf(double out_ptr[], const double in_ptr[], long nbr_spl) noexcept;

    hiir_FORCEINLINE void process_sample_split(__m256d &low, __m256d &high, __m256d input) noexcept;
...
private:
    ...
    typedef std::array<StageDataF64Avx, NBR_COEFS + 2>
    ...
    hiir_FORCEINLINE EvenOdd process_2_paths(__m256d input) noexcept;
    ...
    hiir_FORCEINLINE static void store_low(double *ptr, __m256d tmp_0, __m256d tmp_1) noexcept;
    hiir_FORCEINLINE static void store_high(double *ptr, __m256d tmp_0, __m256d tmp_1) noexcept;
    hiir_FORCEINLINE static void bypass(double *, __m256d, __m256d) noexcept {}

    FilterBiPhase _bifilter;
    alignas(32) double _prev[_nbr_chn];
    int _phase;  // 0 or 1
