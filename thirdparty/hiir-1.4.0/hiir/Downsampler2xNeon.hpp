...
template <int NC>
Downsampler2xNeon<NC>::Downsampler2xNeon() noexcept : _filter()
{
    for(int i = 0; i < _nbr_stages + 1; ++i) { storea(_filter[i]._mem, vdup_n_f32(0)); }
    if((NBR_COEFS & 1) != 0)
    {
...
template <int NC>
void Downsampler2xNeon<NC>::process_block(float out_ptr[], const float in_ptr[],
...
template <int NC>
void Downsampler2xNeon<NC>::process_sample_split(float &low, float &high,
...
template <int NC>
void Downsampler2xNeon<NC>::process_block_split(float out_l_ptr[], float out_h_ptr[],
...
long Downsampler2xNeon<NC>::process_block_quad(float out_l_ptr[], float out_h_ptr[],
...
template <int NC>
void Downsampler2xNeon<NC>::store_low(float *ptr, float32x4_t even, float32x4_t odd,
...
template <int NC>
void Downsampler2xNeon<NC>::store_high(float *ptr, float32x4_t even, float32x4_t odd,
                                       float32x4_t half) noexcept
{
