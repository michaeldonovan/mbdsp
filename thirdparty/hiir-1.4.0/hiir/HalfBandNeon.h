...
public:
    ...
    HalfBandNeon<NC> &operator=(const HalfBandNeon<NC> &other) = default;
    HalfBandNeon<NC> &operator=(HalfBandNeon<NC> &&other) = default;

    void set_coefs(const double coef_arr[]) noexcept;

    hiir_FORCEINLINE float process_sample(float input) noexcept;
    void process_block(float out_ptr[], const float in_ptr[], long nbr_spl) noexcept;

    hiir_FORCEINLINE float process_sample_hpf(float input) noexcept;
    void process_block_hpf(float out_ptr[], const float in_ptr[], long nbr_spl) noexcept;

    hiir_FORCEINLINE void process_sample_split(float &low, float &hi, float input) noexcept;
...
private:
    ...
    typedef std::array<StageDataNeonV4, _nbr_stages + 1>
        Filter;  // Stage 0 contains only input memory
    typedef std::array<Filter, _nbr_phases> FilterBiPhase;

    inline void set_single_coef(int index, double coef) noexcept;

    hiir_FORCEINLINE float32x2_t process_2_paths(float input) noexcept;
    template <typename FL, typename FH>
    hiir_FORCEINLINE long process_block_quad(float out_l_ptr[], float out_h_ptr[],
                                             const float in_ptr[], long nbr_spl, FL fnc_l,
                                             FH fnc_h) noexcept;

    hiir_FORCEINLINE static void store_low(float *ptr, float32x4_t even, float32x4_t odd,
                                           float32x4_t half) noexcept;
    hiir_FORCEINLINE static void store_high(float *ptr, float32x4_t even, float32x4_t odd,
                                            float32x4_t half) noexcept;
    hiir_FORCEINLINE static void bypass(float *, float32x4_t, float32x4_t, float32x4_t) noexcept {}
