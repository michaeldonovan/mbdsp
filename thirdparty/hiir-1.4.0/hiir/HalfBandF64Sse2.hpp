...
template <int NC>
HalfBandF64Sse2<NC>::HalfBandF64Sse2() noexcept : _bifilter(), _prev(0), _phase(0)
{
    for(int phase = 0; phase < _nbr_phases; ++phase)
    {
        ...
        if(NBR_COEFS < _nbr_stages * 2) { _bifilter[phase][_nbr_stages]._coef[0] = 1; }
    }
...
template <int NC>
void HalfBandF64Sse2<NC>::process_block(double out_ptr[], const double in_ptr[],
...
template <int NC>
void HalfBandF64Sse2<NC>::process_block_hpf(double out_ptr[], const double in_ptr[],
                                            long nbr_spl) noexcept
{
    assert(out_ptr != nullptr);
    assert(out_ptr <= in_ptr || out_ptr >= in_ptr + nbr_spl * _nbr_chn);
...
template <int NC>
void HalfBandF64Sse2<NC>::process_block_split(double out_l_ptr[], double out_h_ptr[],
...
std::array<double, 2> HalfBandF64Sse2<NC>::process_2_paths(double input) noexcept
{
    ...
    StageProcF64Sse2<_nbr_stages>::process_sample_pos(x, &_bifilter[_phase][0]);
...
void HalfBandF64Sse2<NC>::process_block_2_paths(double out_l_ptr[], double out_h_ptr[],
                                                ...
                                                FH fnc_h) noexcept
{
    ...
    while(pos < end)
    {
        ...
        StageProcF64Sse2<_nbr_stages>::process_sample_pos(x_0, _bifilter[0].data());
        StageProcF64Sse2<_nbr_stages>::process_sample_pos(x_1, _bifilter[1].data());
