...
template <int NC>
Downsampler2x4Neon<NC>::Downsampler2x4Neon() noexcept : _filter()
{
    for(int i = 0; i < NBR_COEFS + 2; ++i) { storea(_filter[i]._coef, vdupq_n_f32(0)); }
...
float32x4_t Downsampler2x4Neon<NC>::process_sample(float32x4_t in_0, float32x4_t in_1) noexcept
{
    ...
    StageProc4Neon<NBR_COEFS>::process_sample_pos(NBR_COEFS, spl_0, spl_1, &_filter[0]);
...
template <int NC>
void Downsampler2x4Neon<NC>::process_block(float out_ptr[], const float in_ptr[],
...
template <int NC>
void Downsampler2x4Neon<NC>::process_sample_split(float32x4_t &low, float32x4_t &high,
                                                  const float in_ptr[8]) noexcept
{
    assert(in_ptr != nullptr);
...
template <int NC>
void Downsampler2x4Neon<NC>::process_sample_split(float32x4_t &low, float32x4_t &high,
                                                  float32x4_t in_0, float32x4_t in_1) noexcept
{
    float32x4_t spl_0 = in_1;
    float32x4_t spl_1 = in_0;
...
template <int NC>
void Downsampler2x4Neon<NC>::process_block_split(float out_l_ptr[], float out_h_ptr[],
...
{
    for(int i = 0; i < NBR_COEFS + 2; ++i) { storea(_filter[i]._mem, vdupq_n_f32(0)); }
}
