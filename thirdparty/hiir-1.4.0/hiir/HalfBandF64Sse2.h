...
public:
    ...
    HalfBandF64Sse2<NC> &operator=(const HalfBandF64Sse2<NC> &other) = default;
    HalfBandF64Sse2<NC> &operator=(HalfBandF64Sse2<NC> &&other) = default;

    void set_coefs(const double coef_arr[]) noexcept;

    hiir_FORCEINLINE double process_sample(double input) noexcept;
    void process_block(double out_ptr[], const double in_ptr[], long nbr_spl) noexcept;

    hiir_FORCEINLINE double process_sample_hpf(double input) noexcept;
    void process_block_hpf(double out_ptr[], const double in_ptr[], long nbr_spl) noexcept;

    hiir_FORCEINLINE void process_sample_split(double &low, double &high, double input) noexcept;
...
private:
    ...
    hiir_FORCEINLINE std::array<double, 2> process_2_paths(double input) noexcept;
    ...
    static hiir_FORCEINLINE void store_low(double *ptr, double even, double odd) noexcept;
    static hiir_FORCEINLINE void store_high(double *ptr, double even, double odd) noexcept;
    static hiir_FORCEINLINE void bypass(double *, double, double) noexcept {}

    // Should be the first member (thus easier to align)
    alignas(16) FilterBiPhase _bifilter;
    double _prev;
    int _phase;  // 0 or 1
