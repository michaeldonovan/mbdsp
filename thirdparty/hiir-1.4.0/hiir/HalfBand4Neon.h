...
public:
    ...
    HalfBand4Neon<NC> &operator=(const HalfBand4Neon<NC> &other) = default;
    HalfBand4Neon<NC> &operator=(HalfBand4Neon<NC> &&other) = default;

    void set_coefs(const double coef_arr[]) noexcept;

    hiir_FORCEINLINE float32x4_t process_sample(float32x4_t input) noexcept;
    void process_block(float out_ptr[], const float in_ptr[], long nbr_spl) noexcept;

    hiir_FORCEINLINE float32x4_t process_sample_hpf(float32x4_t input) noexcept;
    void process_block_hpf(float out_ptr[], const float in_ptr[], long nbr_spl) noexcept;

    hiir_FORCEINLINE void process_sample_split(float32x4_t &low, float32x4_t &high,
...
private:
    ...
    typedef std::array<StageDataNeonV4, NBR_COEFS + 2>
    ...
    hiir_FORCEINLINE EvenOdd process_2_paths(float32x4_t input) noexcept;
    template <typename FL, typename FH>
    hiir_FORCEINLINE void process_block_2_paths(float out_l_ptr[], float out_h_ptr[],
                                                const float in_ptr[], long nbr_spl, FL fnc_l,
                                                FH fnc_h) noexcept;

    hiir_FORCEINLINE static void store_low(float *ptr, float32x4_t tmp_0,
                                           float32x4_t tmp_1) noexcept;
    hiir_FORCEINLINE static void store_high(float *ptr, float32x4_t tmp_0,
                                            float32x4_t tmp_1) noexcept;
    hiir_FORCEINLINE static void bypass(float *, float32x4_t, float32x4_t) noexcept {}

    FilterBiPhase _bifilter;
    alignas(16) float _prev[_nbr_chn];
    int _phase;  // 0 or 1
