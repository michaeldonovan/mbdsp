...
template <int NC>
HalfBand8F64Avx512<NC>::HalfBand8F64Avx512() noexcept : _bifilter(), _phase(0)
{
...
template <int NC>
void HalfBand8F64Avx512<NC>::process_block(double out_ptr[], const double in_ptr[],
                                           long nbr_spl) noexcept
{
    assert(out_ptr != nullptr);
    assert(out_ptr <= in_ptr || out_ptr >= in_ptr + nbr_spl * _nbr_chn);

    process_block_2_paths(out_ptr, nullptr, in_ptr, nbr_spl, store_low, bypass);
}
...
template <int NC>
void HalfBand8F64Avx512<NC>::process_block_hpf(double out_ptr[], const double in_ptr[],
                                               long nbr_spl) noexcept
{
    assert(out_ptr != nullptr);
    assert(out_ptr <= in_ptr || out_ptr >= in_ptr + nbr_spl * _nbr_chn);

    process_block_2_paths(nullptr, out_ptr, in_ptr, nbr_spl, bypass, store_high);
}
...
template <int NC>
void HalfBand8F64Avx512<NC>::process_sample_split(__m512d &low, __m512d &high,
                                                  __m512d input) noexcept
{
    const auto path_arr = process_2_paths(input);
...
template <int NC>
void HalfBand8F64Avx512<NC>::process_block_split(double out_l_ptr[], double out_h_ptr[],
                                                 const double in_ptr[], long nbr_spl) noexcept
{
    ...
    process_block_2_paths(out_l_ptr, out_h_ptr, in_ptr, nbr_spl, store_low, store_high);
}
...
template <int NC>
typename HalfBand8F64Avx512<NC>::EvenOdd HalfBand8F64Avx512<NC>::process_2_paths(
    __m512d input) noexcept
{
    ...
    StageProc8F64Avx512<NBR_COEFS>::process_sample_pos(NBR_COEFS, path_arr._e, path_arr._o,
                                                       &_bifilter[_phase][0]);
...
void HalfBand8F64Avx512<NC>::process_block_2_paths(double out_l_ptr[], double out_h_ptr[],
                                                   ...
                                                   FH fnc_h) noexcept
{
    ...
    while(pos < end)
    {
        ...
        auto tmp_0 = input_0;
        auto tmp_1 = prev;
        StageProc8F64Avx512<NBR_COEFS>::process_sample_pos(NBR_COEFS, tmp_0, tmp_1,
                                                           &_bifilter[0][0]);

        auto tmp_2 = input_1;
        auto tmp_3 = input_0;  // prev
        StageProc8F64Avx512<NBR_COEFS>::process_sample_pos(NBR_COEFS, tmp_2, tmp_3,
                                                           &_bifilter[1][0]);
