...
template <int NC, typename DT, int NCHN>
void Upsampler2xTpl<NC, DT, NCHN>::process_sample(DataType &out_0, DataType &out_1,
                                                  DataType input) noexcept
{
    DataType even = input;
    DataType odd = input;
    StageProcTpl<NBR_COEFS, DataType>::process_sample_pos(NBR_COEFS, even, odd, _filter.data());
    out_0 = even;
    out_1 = odd;
}
...
template <int NC, typename DT, int NCHN>
void Upsampler2xTpl<NC, DT, NCHN>::process_block(DataType out_ptr[], const DataType in_ptr[],
