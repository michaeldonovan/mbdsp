...
float Downsampler2xSse<NC>::process_sample(const float in_ptr[2]) noexcept
{
    ...
    auto x = _mm_loadl_pi(_mm_setzero_ps(), reinterpret_cast<const __m64 *>(in_ptr));
...
template <int NC>
void Downsampler2xSse<NC>::process_block(float out_ptr[], const float in_ptr[],
                                         long nbr_spl) noexcept
{
    ...
    {
        auto x = _mm_loadl_pi(half, reinterpret_cast<const __m64 *>(in_ptr + pos * 2));
        StageProcSseV2<_nbr_stages>::process_sample_pos(x, _filter.data());
        x = _mm_add_ss(x, _mm_shuffle_ps(x, x, 1));
        x = _mm_mul_ss(x, half);
...
template <int NC>
void Downsampler2xSse<NC>::process_sample_split(float &low, float &high,
...
template <int NC>
void Downsampler2xSse<NC>::process_block_split(float out_l_ptr[], float out_h_ptr[],
...
long Downsampler2xSse<NC>::process_block_quad(float out_l_ptr[], float out_h_ptr[],
