...
template <int NC>
Upsampler2x3dnow<NC>::Upsampler2x3dnow() noexcept : _filter()
{
...
template <int NC>
void Upsampler2x3dnow<NC>::process_block(float out_ptr[], const float in_ptr[],
                                         long nbr_spl) noexcept
{
    ...
    __asm pop eax
        ...
        __asm
    {
        inc eax movq[edx + CURR_CELL + 1 * 8], mm0 movd[edi + eax * 8 + 4], mm0 punpckhdq mm0,
            mm0 movd[edi + eax * 8],
            mm0
...
template <>
void Upsampler2x3dnow<8>::process_block(float out_ptr[], const float in_ptr[],
                                        long nbr_spl) noexcept
{
    StageData3dnow *filter_ptr = &_filter[0];
