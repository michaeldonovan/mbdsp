...
public:
    ...
    PhaseHalfPiSse<NC> &operator=(const PhaseHalfPiSse<NC> &other) = default;
    PhaseHalfPiSse<NC> &operator=(PhaseHalfPiSse<NC> &&other) = default;

    void set_coefs(const double coef_arr[]) noexcept;

    hiir_FORCEINLINE void process_sample(float &out_0, float &out_1, float input) noexcept;
    void process_block(float out_0_ptr[], float out_1_ptr[], const float in_ptr[],
...
private:
    ...
    hiir_FORCEINLINE long process_block_quad(float out_0_ptr[], float out_1_ptr[],
