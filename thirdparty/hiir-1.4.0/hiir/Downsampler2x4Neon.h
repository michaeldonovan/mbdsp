...
public:
    ...
    Downsampler2x4Neon<NC> &operator=(const Downsampler2x4Neon<NC> &other) = default;
    Downsampler2x4Neon<NC> &operator=(Downsampler2x4Neon<NC> &&other) = default;

    void set_coefs(const double coef_arr[]) noexcept;

    hiir_FORCEINLINE float32x4_t process_sample(const float in_ptr[8]) noexcept;
    hiir_FORCEINLINE float32x4_t process_sample(float32x4_t in_0, float32x4_t in_1) noexcept;
