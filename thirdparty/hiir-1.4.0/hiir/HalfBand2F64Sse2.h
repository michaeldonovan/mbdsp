...
public:
    ...
    HalfBand2F64Sse2<NC> &operator=(const HalfBand2F64Sse2<NC> &other) = default;
    HalfBand2F64Sse2<NC> &operator=(HalfBand2F64Sse2<NC> &&other) = default;

    void set_coefs(const double coef_arr[]) noexcept;

    hiir_FORCEINLINE __m128d process_sample(__m128d input) noexcept;
    void process_block(double out_ptr[], const double in_ptr[], long nbr_spl) noexcept;

    hiir_FORCEINLINE __m128d process_sample_hpf(__m128d input) noexcept;
    void process_block_hpf(double out_ptr[], const double in_ptr[], long nbr_spl) noexcept;

    hiir_FORCEINLINE void process_sample_split(__m128d &low, __m128d &high, __m128d input) noexcept;
...
private:
    ...
    typedef std::array<StageDataF64Sse2, NBR_COEFS + 2>
    ...
    hiir_FORCEINLINE EvenOdd process_2_paths(__m128d input) noexcept;
    ...
    hiir_FORCEINLINE static void store_low(double *ptr, __m128d tmp_0, __m128d tmp_1) noexcept;
    hiir_FORCEINLINE static void store_high(double *ptr, __m128d tmp_0, __m128d tmp_1) noexcept;
    hiir_FORCEINLINE static void bypass(double *, __m128d, __m128d) noexcept {}

    FilterBiPhase _bifilter;
    alignas(16) double _prev[_nbr_chn];
    int _phase;  // 0 or 1
