...
public:
    using MaxResClock =
        typename std::conditional<std::ratio_less<std::chrono::system_clock::period,
                                                  std::chrono::steady_clock::period>::value,
                                  std::chrono::system_clock, std::chrono::steady_clock>::type;

    TimerAccurate() = default;
    virtual ~TimerAccurate() = default;

    hiir_FORCEINLINE void start() noexcept;
    hiir_FORCEINLINE void stop() noexcept;
    MaxResClock::duration inline get_best_duration() const noexcept;
...
private:
    MaxResClock::time_point _start;
    MaxResClock::time_point _end;
    MaxResClock::duration _best = MaxResClock::duration::max();
...
private:
    ...
    TimerAccurate &operator=(const TimerAccurate &other) = delete;
    TimerAccurate &operator=(TimerAccurate &&other) = delete;
