...
template <class TO>
int TestHalfBand<TO>::perform_test(TO &filter, const double coef_arr[_nbr_coefs],
                                   const SweepingSine &ss, const char *type_0, double transition_bw,
                                   double stopband_at)
{
    ...
    const char *datatype_0 = std::is_same<DataType, double>::value  ? "double"
                             : std::is_same<DataType, float>::value ? "float"
                                                                    : "<unknown type>";
    printf("Test: HalfBand, %s, %d chn, %s implementation, %d coefficients.\n", datatype_0,
           _nbr_chn, type_0, _nbr_coefs);

    const auto len = ss.get_len();
    const auto src = ResultCheck<DataType>::generate_test_signal(ss, len, _nbr_chn);
    ...
    {
        ret_val = check_band(dst_chk, dest_lo, false, chn, ss, type_0, transition_bw, stopband_at,
    ...
    if(ret_val == 0)
    {
        ...
        for(int chn = 0; chn < _nbr_chn && ret_val == 0; ++chn)
        {
            ret_val = check_band(dst_chk, dest_hi, true, chn, ss, type_0, transition_bw,
                                 stopband_at, "hb_%02d_%s_%dx-%01d_hpf.raw");
        }
    }
    ...
    if(ret_val == 0)
    {
        ...
        for(bs.start(len); bs.is_continuing(); bs.set_next_block())
        {
            const long b_pos = bs.get_pos();
            const long b_len = bs.get_len();
            filter.process_block_split(&dest_lo[b_pos * _nbr_chn], &dest_hi[b_pos * _nbr_chn],
                                       &src[b_pos * _nbr_chn], b_len);
        }
        printf("Done.\n");
    }
    ...
    for(int chn = 0; chn < _nbr_chn && ret_val == 0; ++chn)
    {
        ret_val = check_band(dst_chk, dest_hi, true, chn, ss, type_0, transition_bw, stopband_at,
...
template <class TO>
int TestHalfBand<TO>::check_band(std::vector<DataType> &dst_chk, const std::vector<DataType> &dest,
                                 bool hi_flag, int chn, const SweepingSine &ss, const char *type_0,
                                 double transition_bw, double stopband_at,
                                 const char *filename_fmt_0)
{
    int ret_val = 0;

    const long len_proc = long(dst_chk.size());
    for(long pos = 0; pos < len_proc; ++pos) { dst_chk[pos] = dest[pos * _nbr_chn + chn]; }

    ret_val = ResultCheck<DataType>::check_halfband(ss, transition_bw, stopband_at, dst_chk.data(),
                                                    hi_flag, 1);

    char filename_0[255 + 1];
    sprintf(filename_0, filename_fmt_0, _nbr_coefs, type_0, _nbr_chn, chn);
    FileOp<DataType>::save_raw_data(filename_0, dst_chk.data(), len_proc, hiir_test_file_resol,
                                    1.f);
