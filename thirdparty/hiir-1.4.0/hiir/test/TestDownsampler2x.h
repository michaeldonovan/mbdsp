...
public:
    ...
    static int perform_test(TO &dspl, const double coef_arr[_nbr_coefs], const SweepingSine &ss,
                            const char *type_0, double transition_bw, double stopband_at);
...
private:
    static int check_band(std::vector<DataType> &dst_chk, const std::vector<DataType> &dest,
                          bool hi_flag, int chn, const SweepingSine &ss, const char *type_0,
                          double transition_bw, double stopband_at, const char *filename_fmt_0);
...
private:
    ...
    ~TestDownsampler2x() = delete;
    TestDownsampler2x<TO> &operator=(const TestDownsampler2x<TO> &other) = delete;
    TestDownsampler2x<TO> &operator=(TestDownsampler2x<TO> &&other) = delete;
