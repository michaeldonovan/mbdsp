...
std::vector<T> ResultCheck<T>::generate_test_signal(const SweepingSine &ss, long len, int nbr_chn)
{
    ...
    fflush(stdout);
    if(nbr_chn == 1) { ss.generate(src.data()); }
    else
    {
        ...
        {
            for(int chn = 0; chn < nbr_chn; ++chn) { src[pos * nbr_chn + chn] = src_base[pos]; }
        }
    }
    printf("Done.\n");
...
template <typename T>
int ResultCheck<T>::check_halfband(const SweepingSine &ss, double bw, double at, const T out_ptr[],
                                   bool hiband_flag, int rate_div)
{
    ...
    if(err_pb_avg > 0.1)
    {
        ...
        printf("Error: abnormal average volume in passband (%f dB).\n", vol_pb_avg_db);
        ret_val = -1;
    }
    ...
    if(err_sb_avg > 0.25)
    {
        ...
        printf("Error: abnormal average volume in stopband (%f dB).\n", vol_sb_avg_db);
        ret_val = -2;
    }
...
int ResultCheck<T>::check_uspl(const SweepingSine &ss, double bw, double at, const T out_ptr[])
{
    ...
    for(long fir_pos = 0; fir_pos < fir_len; ++fir_pos)
    {
        ...
        if(fir_pos == fir_mid) { fir[fir_pos] = float(f_fir - 1); }
        else
        {
    ...
    for(long pos = 0; pos < len_ana; ++pos)
    {
        ...
        double val_fir = 0;
        for(long k = 0; k < fir_len; ++k) { val_fir += out_ptr[pos + k] * fir[k]; }
    ...
    if(err_avg > 0.1)
    {
        ...
        printf("Error: abnormal average volume (%f dB).\n", vol_avg_db);
        ret_val = -1;
    }
...
// We should take group delay into account
template <typename T>
int ResultCheck<T>::check_phase(const SweepingSine &ss, double bw, const T out_0_ptr[],
                                const T out_1_ptr[])
{
...
template <typename T>
int ResultCheck<T>::check_delay(const T out_ptr[], const T ref_ptr[], long nbr_spl,
                                double dly_expect, double f_fs)
{
    assert(out_ptr != nullptr);
    ...
    const double dly_found = find_delay(out_ptr, ref_ptr, nbr_spl, skip, start);
    const double err = fabs(dly_found - dly_expect);
    if(err > max_dly_err)
    {
        printf("Error: abnormal delay error (%f spl, expected %f spl).\n", dly_found, dly_expect);
        ret_val = -1;
    }
...
template <typename T>
double ResultCheck<T>::find_delay(const T tst_ptr[], const T ref_ptr[], long len, long skip,
                                  int start)
{
    ...
    for(int dly_tst = start; dly_tst <= dly_max; ++dly_tst)
    {
        ...
        if(sum <= sel_val)
        {
            if(dly_tst < dly_min + 2) { return double(sel_dly); }
            const double frac = find_extremum_pos_parabolic(dly_arr[dly_tst - 2 - dly_min],
                                                            dly_arr[dly_tst - 1 - dly_min], sum);
