...
template <typename T>
inline int FileOp<T>::save_raw_data(const char *filename_0, const T src_ptr[], long nbr_spl,
                                    int bits, float scale)
{
    ...
    else
    {
        ...
        for(long block_pos = 0; block_pos < nbr_spl && ret_val == 0; block_pos += BLOCK_LEN)
        {
            std::array<uint8_t, BLOCK_LEN * max_bytes> buf;
            const long block_size = std::min(nbr_spl - block_pos, long(BLOCK_LEN));
            ...
            if(fwrite(&buf[0], nbr_bytes * block_size, 1, f_ptr) != 1) { ret_val = -2; }
        }
    }
...
template <typename T>
inline int FileOp<T>::save_raw_data_stereo(const char *filename_0, const T src_0_ptr[],
                                           const T src_1_ptr[], long nbr_spl, int bits, float scale)
{
    assert(filename_0 != nullptr);
    assert(filename_0[0] != '\0');
...
template <typename T>
inline int FileOp<T>::save_raw_data(const char* /*filename_0*/, const T /*src_ptr*/[],
                                    long /*nbr_spl*/, int /*bits*/, float /*scale*/)
{
    return 0;
}
...
template <typename T>
inline int FileOp<T>::save_raw_data_stereo(const char* /*filename_0*/, const T /*src_0_ptr*/[],
                                           const T /*src_1_ptr*/[], long /*nbr_spl*/, int /*bits*/,
                                           float /*scale*/)
{
    return 0;
}
