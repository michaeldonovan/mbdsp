...
{
    const double dly_ref = filter._delay + PolyphaseIir2Designer::compute_phase_delay(
    ...
    {
        for(int c = 0; c < _nbr_chn; ++c) { src[k * _nbr_chn + c] = src_m[k]; }
    }
    filter.clear_buffers();
    filter.process_block(dst.data(), src.data(), _len);
    for(long k = 0; k < _len; ++k) { dst_m[k] = dst[k * _nbr_chn]; }

    int ret_val =
        ResultCheck<DataType>::check_delay(dst_m.data(), src_m.data(), _len, dly_ref, _f_fs);
...
int TestDelay<TO>::test_downsampler(TO &filter, const double coef_arr[_nbr_coefs])
{
    ...
    {
        for(int c = 0; c < _nbr_chn; ++c) { src[k * _nbr_chn + c] = src_m2[k]; }
    }
    filter.clear_buffers();
    filter.process_block(dst.data(), src.data(), _len);
    for(long k = 0; k < _len; ++k)
    ...
    int ret_val = ResultCheck<DataType>::check_delay(dst_m.data(), src_m.data(), _len,
                                                     dly_ref * 0.5, _f_fs * 2);
...
int TestDelay<TO>::test_upsampler(TO &filter, const double coef_arr[_nbr_coefs])
{
    ...
    for(long k = 0; k < len_src; ++k)
    {
        for(int c = 0; c < _nbr_chn; ++c) { src[k * _nbr_chn + c] = src_m[k * 2]; }
    }
    filter.clear_buffers();
...
int TestDelay<TO>::test_phaser(TO &filter, const double coef_arr[_nbr_coefs])
{
    ...
    for(long k = 0; k < _len; ++k) { dst_m[k] = dst_1[k * _nbr_chn]; }
