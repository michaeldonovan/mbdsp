...
public:
    static std::vector<T> generate_test_signal(const SweepingSine &ss, long len, int nbr_chn);

    static int check_halfband(const SweepingSine &ss, double bw, double at, const T out_ptr[],
                              bool hiband_flag, int rate_div);
    static int check_uspl(const SweepingSine &ss, double bw, double at, const T out_ptr[]);
    static int check_phase(const SweepingSine &ss, double bw, const T out_0_ptr[],
                           const T out_1_ptr[]);

    static int check_delay(const T out_ptr[], const T ref_ptr[], long nbr_spl, double dly_expect,
                           double f_fs);
...
private:
    ...
    ResultCheck<T> &operator=(const ResultCheck<T> &other) = delete;
    ResultCheck<T> &operator=(ResultCheck<T> &&other) = delete;
