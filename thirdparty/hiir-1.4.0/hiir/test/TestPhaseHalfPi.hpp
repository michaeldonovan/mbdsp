...
template <class TO>
int TestPhaseHalfPi<TO>::perform_test(TO &phaser, const double coef_arr[_nbr_coefs],
                                      ...
                                      double transition_bw)
{
    ...
    const char *datatype_0 = std::is_same<DataType, double>::value  ? "double"
                             : std::is_same<DataType, float>::value ? "float"
                                                                    : "<unknown type>";
    printf("Test: PhaseHalfPi, %s, %d chn, %s implementation, %d coefficients.\n", datatype_0,
    ...
    for(bs.start(len); bs.is_continuing(); bs.set_next_block())
    {
        ...
        phaser.process_block(&dest_0[idx], &dest_1[idx], &src[idx], b_len);
    }
    printf("Done.\n");
    ...
    for(int chn = 0; chn < _nbr_chn && ret_val == 0; ++chn)
    {
        ...
        ret_val =
            ResultCheck<DataType>::check_phase(ss, transition_bw, &dst_chk_0[0], &dst_chk_1[0]);

        char filename_0[255 + 1];
        sprintf(filename_0, "phaser_%02d_%s_%dx-%01d.raw", _nbr_coefs, type_0, _nbr_chn, chn);
        FileOp<DataType>::save_raw_data_stereo(filename_0, dst_chk_0.data(), dst_chk_1.data(), len,
                                               hiir_test_file_resol, 1.f);
    }
