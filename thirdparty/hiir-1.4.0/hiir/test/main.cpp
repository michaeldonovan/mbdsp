...
#if defined(_MSC_VER)
#pragma warning( \
    4 : 4786)  // "identifier was truncated to '255' characters in the debug information"
...
static int __cdecl HIIR_debug_alloc_hook_cb(int alloc_type, void *user_data_ptr, size_t size,
                                            int block_type, long request_nbr,
                                            const unsigned char *filename_0, int line_nbr)
{
...
static void HIIR_main_prog_init()
{
...
#if defined(_MSC_VER) && !defined(NDEBUG)
    {
        ...
        ::_CrtSetDbgFlag(old_flags | (1 * _CRTDBG_LEAK_CHECK_DF) | (1 * _CRTDBG_CHECK_ALWAYS_DF));
...
int main(int /* argc */, char * /* argv */[])
{
    ...
    try
    {
        ...
        if(ret_val == 0) { ret_val = HIIR_test_functionalities(); }
        if(ret_val == 0) { ret_val = HIIR_test_speed(); }

        if(ret_val == 0) { printf("All tests passed successfully.\n\n"); }
