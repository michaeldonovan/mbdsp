...
public:
    ...
    Downsampler2xNeonOld<NC> &operator=(const Downsampler2xNeonOld<NC> &other) = default;
    Downsampler2xNeonOld<NC> &operator=(Downsampler2xNeonOld<NC> &&other) = default;

    void set_coefs(const double coef_arr[]) noexcept;

    hiir_FORCEINLINE float process_sample(const float in_ptr[2]) noexcept;
    void process_block(float out_ptr[], const float in_ptr[], long nbr_spl) noexcept;

    hiir_FORCEINLINE void process_sample_split(float &low, float &high,
