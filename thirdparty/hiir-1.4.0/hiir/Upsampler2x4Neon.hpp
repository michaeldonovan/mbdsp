...
template <int NC>
Upsampler2x4Neon<NC>::Upsampler2x4Neon() noexcept : _filter()
{
...
template <int NC>
void Upsampler2x4Neon<NC>::process_sample(float32x4_t &out_0, float32x4_t &out_1,
                                          float32x4_t input) noexcept
{
    float32x4_t even = input;
    float32x4_t odd = input;
    StageProc4Neon<NBR_COEFS>::process_sample_pos(NBR_COEFS, even, odd, &_filter[0]);
    out_0 = even;
    out_1 = odd;
}
...
template <int NC>
void Upsampler2x4Neon<NC>::process_block(float out_ptr[], const float in_ptr[],
                                         long nbr_spl) noexcept
{
    assert(out_ptr != nullptr);
    assert(in_ptr != nullptr);
    assert(out_ptr >= in_ptr + nbr_spl * _nbr_chn || in_ptr >= out_ptr + nbr_spl * _nbr_chn);
    assert(nbr_spl > 0);
