...
float32x4_t load4a(const float *ptr) noexcept { return vld1q_f32(ptr); }

float32x4_t load4u(const float *ptr) noexcept
{
    return vreinterpretq_f32_u8(vld1q_u8(reinterpret_cast<const uint8_t *>(ptr)));
}

float32x2_t load2a(const float *ptr) noexcept { return vld1_f32(ptr); }

float32x2_t load2u(const float *ptr) noexcept
{
    return vreinterpret_f32_u8(vld1_u8(reinterpret_cast<const uint8_t *>(ptr)));
}

void storea(float *ptr, float32x4_t x) noexcept { vst1q_f32(ptr, x); }
...
void storea(float *ptr, float32x2_t x) noexcept { vst1_f32(ptr, x); }
